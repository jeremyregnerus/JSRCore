...
namespace JSR.BaseClassLibrary
{
    ...
    public class BaseNode<T> : BaseClass
    {
        private T item;
        ...
        public BaseNode(T item, BaseNode<T> parent) : this(item)
        {
            Parent = parent;
        }
        ...
        public BaseNode(T item, IEnumerable<BaseNode<T>> children) : this(item)
        {
            Children = new BaseCollection<BaseNode<T>>(children);
        }
        ...
        /// <param name="parent">Parent Node of this Node.</param>
        /// <param name="children">Children Nodes of this Node.</param>
        public BaseNode(T item, BaseNode<T> parent, IEnumerable<BaseNode<T>> children) : this(item, parent)
        {
            Children = new BaseCollection<BaseNode<T>>(children);
        }
        ...
        /// </summary>
        public BaseCollection<BaseNode<T>> Children { get; } = new BaseCollection<BaseNode<T>>();

        /// <summary>
        /// Gets or sets the Parent to this Node.
        /// </summary>
        public BaseNode<T> Parent { get => parent; set => SetValue(value, ref parent); }
        ...
        public void AddChild(T item, bool unique)
        {
            ...
            Children.Add(new BaseNode<T>(item) { Parent = this });
        }
        ...
        public void AddChild(BaseNode<T> node, bool unique)
        {
            if (node.Parent == this)
            {
                return;
        ...
        /// <param name="unique">Only add items if they do not exist in the list already.</param>
        public void AddRange(IEnumerable<BaseNode<T>> nodes, bool unique)
        {
            foreach (BaseNode<T> node in nodes)
            {
        ...
        public BaseNode<T> GetCopy(BaseNode<T> parent, bool withChildren = true)
        {
            ...
            if (withChildren)
            {
                foreach (BaseNode<T> child in Children)
                {
                    copy.AddChild(child.GetCopy(copy), false);
                }
            }
